...
//[Bind("FirstName,LastName,CreatedBy,CompanyName, CreatedDateTime,ImageUrl, User, UserId") ]
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(Guid id,[Bind("FirstName,LastName,CreatedBy,CompanyName, CreatedDateTime,ImageUrl, User, UserId") ]Employee employee, IFormFile? imageFile)
    {
        employee.ModifiedDateTime = DateTime.Now;
        var user = _um.GetUserAsync(User).Result;
        employee.ModifiedBy = user.CompanyName;
...
// POST: Movies/Delete/5
            [HttpPost, ActionName("Delete")]
            [ValidateAntiForgeryToken]
            public async Task<IActionResult> Delete(Guid id,[Bind("FirstName,LastName,CreatedBy,CompanyName, CreatedDateTime,ImageUrl, User, UserId") ]Employee employee, IFormFile? imageFile)
            {
                employee.ModifiedDateTime = DateTime.Now;
                var user = _um.GetUserAsync(User).Result;
                employee.ModifiedBy = user.CompanyName;
